...
contract Ai is MappableToken {
	function __Ai_init(address creator_, address factory_, address mine_, address fund_, address team_, address market_, address contribution_) external initializer {
        __Context_init_unchained();
		__ERC20_init_unchained("AiSwap.io Governance Token", "Ai");
		__MappableToken_init_unchained(factory_, creator_);
		__Ai_init_unchained(mine_, fund_, team_, market_, contribution_);
	}
	
	function __Ai_init_unchained(address mine_, address fund_, address team_, address market_, address contribution_) public initializer {
		...
		_mint(contribution_,    25_000_000 * 10 ** uint256(decimals()));
	}
}
